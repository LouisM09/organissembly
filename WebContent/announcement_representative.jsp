...
<div id="id05" class="modal">
  <form class="modal-content animate" action="${pageContext.request.contextPath}/add_announcement" method="post">
    <div class="imgcontainer">
    ...
    <div class="container">
      <label for="announcementTitle"><b>Announcement Title</b></label>
      <input type="text" placeholder="Enter Announcement Title" name="announcementTitle" required>

      <label for="announcementBody"><b>Announcement Details</b></label>
      <textarea rows=4 cols=40 placeholder="Enter details about Announcement" name="announcementBody" id="announcementBody" required></textarea>
      
      <label for="announcementPostedBy"><b>Announcement Posted By: </b></label>
      <select name="announcementPostedBy" id="announcementPostedBy" style="width:200px" required>
      ...
      <input type="hidden" name="dateCreated" id="dateC"/>
      <button type="submit" onclick="setValues()">Add Announcement</button>
...
<div id="id06" class="modal">
  <form class="modal-content animate" action="${pageContext.request.contextPath}/update_announcement" method="post">
    <div class="imgcontainer">
    ...
    <div class="container">
      <label for="editTitle"><b>Announcement Title</b></label>
      <input type="text" name="editTitle" id="editTitle" required>

      <label for="editBody"><b>Announcement Details</b></label>
      <textarea rows=4 cols=40 name="editBody" id="editBody" required></textarea>
      
      <label for="editPostedBy"><b>Announcement Posted By: </b></label>
      <select name="editPostedBy" id="editPostedBy" style="width:200px" required>
		<option value="org_presidentId">Organization President</option>
		<option value="org_secretaryId">Organization Secretary</option>
		<option value="org_proId">Organization P.R.O.</option>
	  </select>
	  
	  <input type="hidden" name="announId" id="announId"/>
	  <input type="hidden" name="editIsApproved" id="editIsApproved"/>
	  <input type="hidden" name="editIsEnabled" id="editIsEnabled"/>
      <input type="hidden" name="dateCreated" id="dateC"/>
      <button type="submit" onclick="setValues()">Update Announcement</button>
...
<div class="navbar">
  ...
  <a href="${pageContext.request.contextPath}/announcements" class="active">Announcements</a>
...
list = (List<Announcement>) session.getAttribute("list");
%>

<div style="width:flex;padding: 0px">
<div class="centeredDiv"><button onclick="document.getElementById('id05').style.display='block'" style="font-size:30px">Add New Announcement</button></div>
...
<c:forEach items="${list}" var="u">
	<div style="width:20%;min-width:300px;align-items:right;background:black;border-top-left-radius:50px;border-top-right-radius: 50px">
		<button class="normalbtn" onclick="document.getElementById('editIsApproved').value='${u.getAnnouncementIsApproved()}';
											document.getElementById('editIsEnabled').value='${u.getAnnouncementIsEnabled()}';
											document.getElementById('announId').value='${u.getAnnouncementId()}';
											document.getElementById('editPostedBy').value='${u.getAnnouncementPostedById()}';
											document.getElementById('editTitle').value='${u.getAnnouncementTitle()}';
											document.getElementById('editBody').value='${u.getAnnouncementBody()}';
											document.getElementById('id06').style.display='block'" style="border-top-left-radius:50px;border-top-right-radius: 50px">Edit Announcement</button>
	</div>
	<div class="centeredDiv" style="background:black;border:2px;border-color:white;border-bottom-left-radius:50px;border-bottom-right-radius: 50px;border-top-right-radius: 50px">
		<div style="width:flex;align-items:center"><h2 style="color:white;align-self:center;">${u.getAnnouncementTitle()}</h2></div>
		<div style="width:flex;">
			<div class="centeredDiv" style="background:#FED250;border-top-left-radius:50px;border-top-right-radius:50px;">
  				...
  				<p style="color:black">${u.getAnnouncementBody()}</p>
			</div>
		</div>
	</div>
</c:forEach>
