...
public class MemberServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//	memberList = dao.getAllRecords(orgId);
		//} else {
		//	memberList = dao.getAllEnabledRecords(orgId);
		//}
		memberList = dao.getAllEnabledRecords(orgId);
		...
		if (userRole.compareTo("Student") == 0) {
			...
			RequestDispatcher rd=request.getRequestDispatcher("member_representative.jsp");
			rd.forward(request, response);
		}
		else if (userRole.compareTo("Organization Adviser") == 0) {
			RequestDispatcher rd=request.getRequestDispatcher("member_adviser.jsp");
			rd.forward(request, response);
		}
		else {
