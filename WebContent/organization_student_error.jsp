...
<div class="topnav" id="myTopnav">
  ...
  <a href="${pageContext.request.contextPath}/org" class="active">Apply</a>
...
<c:forEach items="${list}" var="u">
	<div style="width:20%;min-width:300px;align-items:right;background:black;border-top-left-radius:50px;border-top-right-radius: 50px">
		<form action="${pageContext.request.contextPath}/member_apply" method="post">
			<input type="hidden" name="member_userId" value="<%=session.getAttribute("userId").toString()%>"/>
			<input type="hidden" name="member_orgId" value="${u.getOrgId()}"/>
			<input type="hidden" name="member_dateJoined" id="dateD"/>
			<h2 style="color:white;align-self:center;padding:20px"></h2>
			<button type="submit" class="normalbtn" onclick="setValues()">Apply membership</button>
		</form>
	</div>
...
function errorMessage() {
	alert("User is already a member or has an existing application");
}
</script>
